-- =============================================
-- Cinema Monarca — Datos semilla v3 (PostgreSQL)
-- =============================================
...
-- Salas
INSERT INTO sala (sala_id, nombre, tipo, capacidad, sucursal_id) VALUES
...
(1,'Oppenheimer','La historia del padre de la bomba atómica contada por Christopher Nolan.',180,'DRAMA'),
(2,'Dune: Parte Dos','Paul Atreides se une a los Fremen para vengar a su familia.',166,'CIENCIA_FICCION'),
(3,'Avatar: El Camino del Agua','La épica continuación de James Cameron regresa al mundo de Pandora.',192,'CIENCIA_FICCION'),
(4,'Spider-Man: No Way Home','El héroe arácnido enfrenta su mayor desafío entre multiversos.',148,'ACCION'),
(5,'El Rey León (Clásico)','La historia de Simba regresa a la gran pantalla en proyección 2D.',88,'ANIMACION'),
(6,'Inception','Un ladrón especializado en el robo de secretos dentro del subconsciente.',148,'THRILLER')
ON CONFLICT DO NOTHING;

-- Funciones
INSERT INTO funcion (funcion_id, movie_id, sala_id, fecha, hora_inicio, precio_boleto, capacidad_total, asientos_disponibles) VALUES
...
-- Resetear secuencias para que no colisionen con los IDs insertados
SELECT setval('usuario_usuario_id_seq',  (SELECT MAX(usuario_id)  FROM usuario));
SELECT setval('cine_cine_id_seq',        (SELECT MAX(cine_id)      FROM cine));
SELECT setval('sucursal_bran_id_seq',    (SELECT MAX(bran_id)      FROM sucursal));
SELECT setval('gestor_manager_id_seq',   (SELECT MAX(manager_id)   FROM gestor));
SELECT setval('cliente_cust_id_seq',     (SELECT MAX(cust_id)      FROM cliente));
SELECT setval('sala_sala_id_seq',        (SELECT MAX(sala_id)      FROM sala));
SELECT setval('pelicula_movie_id_seq',   (SELECT MAX(movie_id)     FROM pelicula));
SELECT setval('funcion_funcion_id_seq',  (SELECT MAX(funcion_id)   FROM funcion));